...
| User Story Title | User Story Body |
| --- | --- |
| 3.4.2. Feature: QR Code Scanning (AM-3.4.2) | User Story: As an attendee, I want to present a dynamic, high-resolution QR code, so that I can enter the venue quickly without the risk of my ticket being stolen via a simple screenshot.<br><br>Actions:<br>- Dynamic Payload: Generate a QR code containing a signed payload of the TicketID + UserAddress + Timestamp + One-Time-Salt.<br>- Short-Lived Validity: Configure the mobile dApp to refresh the QR payload every 30 seconds, ensuring that only "live" tickets are valid for scanning.<br>- Fast-Path Processing: Optimize the scanner app to utilize Sui's Owned-Object transaction lane, hitting finality in <400ms.<br><br>Deliverable: A fraud-proof, dynamic QR generation and scanning engine. |

### 3.4.3. Feature: Gated Content (AM-3.4.3)

| User Story Title | User Story Body |
| --- | --- |
| 3.4.3. Feature: Gated Content (AM-3.4.3) | User Story: As an attendee, I want my physical check-in to unlock exclusive digital content (live backstage feeds, exclusive interviews, post-event recordings), so that my ticket provides enhanced value during and after the event based on verified attendance.<br><br>Actions:<br>- Redemption-Locked Encryption: Encrypt exclusive post-check-in assets stored on Walrus using Sui Seal with redemption-based policies.<br>- Policy-Based Decryption: Set an on-chain Seal policy that only releases decryption fragments after the ticket's is_redeemed flag is set to true via the redeem_ticket function (AM-3.1.1).<br>- Attendance Verification: Verify both ticket ownership AND redemption status before granting access to premium content.<br>- Session Gating: Use zkLogin to ensure that only the verified account owner can access the redemption-locked content.<br>- Distinct from Pre-Event Content: Unlike AM-3.3.2 (accessible with ticket ownership), this content requires physical venue check-in to unlock.<br>- Cross-Reference: Integrates with AM-3.1.1 (Check-in Procedures) and AM-3.2.1 (Attendance Badge as additional proof).<br><br>Deliverable: A secure content portal where physical attendance redemption is the cryptographic "key" to unlock exclusive materials. |
